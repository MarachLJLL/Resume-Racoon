...
class TextInput {
    ...
    async fillShortAns() {
        if (!this.inputElement.parentElement.parentElement) {
            return
        }
...
// Example usage
function getAllInputsAndTextareasIncludingIframes() {
    // Collect all input and textarea elements from the main document
    const allElements = [...document.querySelectorAll('input, textarea')];

    // Collect all iframes
    const iframes = document.querySelectorAll('iframe');

    // For each iframe, try to get its document and query its inputs and textareas
...
fillText = async () => {
    ...
    const inputElements = getAllInputsAndTextareasIncludingIframes();
    const textInputs = []
